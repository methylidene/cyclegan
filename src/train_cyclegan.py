...
class UnpairedDataset(Dataset):
    def __init__(self, root_dir, phase='train', transform=None):
        ...
        self.A_paths = sorted([os.path.join(self.dir_A, f) for f in os.listdir(self.dir_A) if f.endswith(('png', 'jpg', 'jpeg'))])
        self.B_paths = sorted([os.path.join(self.dir_B, f) for f in os.listdir(self.dir_B) if f.endswith(('png', 'jpg', 'jpeg'))])
...
# ================= 3. 网络架构定义 (使用你原有的 ResNet 和 PatchGAN) =================
...
def main():
    ...
    BATCH_SIZE = 4      # 3090建议设为4，如果是4090可以尝试设为8
    EPOCHS = 100        # 全量数据 100 Epoch 足够了
    ...
    # 如果有存档的 scheduler 状态，也恢复（可选，由于 LambdaLR 依据 epoch 步进，重新定义也行）
    ...
    try:
        for epoch in range(start_epoch, EPOCHS):
            for i, batch in enumerate(dataloader):
                ...
                if i % SAMPLE_INTERVAL == 0:
                    ...
                    print(f"[Epoch {epoch}/{EPOCHS}] [Batch {i}/{len(dataloader)}] [D loss: {loss_D_A.item()+loss_D_B.item():.4f}] [G loss: {loss_G.item():.4f}] [LR: {current_lr:.6f}]")
        ...
        checkpoint_state['epoch'] = epoch  # 保存中止时的 epoch 进度
