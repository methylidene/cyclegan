...
import torchvision.utils as vutils # 修改了这里，使用 vutils 更方便生成 grid
...
def main():
    ...
    # 🌟 新增：TensorBoard 日志目录 (对齐 AutoDL 默认监听的 /root/tf-logs)
    ...
    TB_LOG_INTERVAL = 100    # 🌟 新增：每 100 个 Batch 往 TensorBoard 打一个点 (画曲线用)
    PRINT_INTERVAL = 400     
    SAVE_IMG_EPOCH_FREQ = 1  # 为了在网页端更快看到效果，预览图频率调成每 1 个 Epoch
    SAVE_EPOCH_FREQ = 20     
    # ------------------------------------------------------
    ...
    start_epoch = 0
    global_step = 0 # 🌟 新增：记录总步数，用于 TensorBoard 的 X 轴
    ...
    try:
        for epoch in range(start_epoch, EPOCHS):
            for i, batch in enumerate(dataloader):
                ...
                if global_step % TB_LOG_INTERVAL == 0:
                    ...
                    # 拆解 Generator 的细分 Loss (非常重要，用来监控是不是 Cycle 崩了)
                ...
                if i % PRINT_INTERVAL == 0:
                    ...
                    print(f"[Epoch {epoch}/{EPOCHS}] [Batch {i}/{len(dataloader)}] "
                          ...
                          f"[G loss: {loss_G.item():.4f}] [LR: {current_lr:.6f}]")

                global_step += 1 # 全局步数加一

            # --- 保存预览图 (硬盘 + TensorBoard 双向保存) ---
            if epoch % SAVE_IMG_EPOCH_FREQ == 0:
                # 把 Real_A, Fake_B, Real_B, Fake_A 拼接到一起
        ...
        writer.close() # 🌟 安全关闭 TensorBoard 写入引擎
